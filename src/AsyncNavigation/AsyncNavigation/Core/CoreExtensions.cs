...
public static class CoreExtensions
{
    ...
    public static P ToParameters<P>(this IEnumerable<KeyValuePair<string, object>> kvp) where P : IParameters, new()
    {
        if (kvp is not null)
        {
            var parameters = new P();
            parameters.AddRange(kvp);
            return parameters;
        }
        throw new ArgumentNullException(nameof(kvp));
    }

    public static IEnumerable<IDisposable> AddTo(this IDisposable disposable, IEnumerable<IDisposable> disposables)
    {
        return disposables.Append(disposable);
    }
