...
internal class RegionIndicatorManager : IRegionIndicatorManager<ContentControl>
{
    ...
    /// </summary>
    public Task ShowContentAsync(NavigationContext navigationContext, object content)
    {
        if (!NavigationOptions.Default.EnableLoadingIndicator)
    ...
    /// </summary>
    public Task ShowErrorAsync(NavigationContext navigationContext, Exception exception)
    {
        if (!NavigationOptions.Default.EnableErrorIndicator)
            return Task.CompletedTask;
    ...
    /// </summary>
    public Task ShowLoadingAsync(NavigationContext navigationContext)
    {
        if (!NavigationOptions.Default.EnableLoadingIndicator)
            return Task.CompletedTask;
        ...
        Task processTask)
    {
        ...
        {
            var loadingContent = _loadingTemplate?.Build(navigationContext.WithStatus(NavigationStatus.InProgress));
            var delayTask = Task.Delay(NavigationOptions.Default.LoadingIndicatorDelay, navigationContext.CancellationToken);
        ...
        {
            await processTask;
            SetIndicatorContent(navigationContext, navigationContext.Target.Value);
        }
    }
